...
import { obtenerPerfil, actualizarDatosPersonales, actualizarPerfilAgricola, cambiarPassword } from '../../api/agricultorService';
...
function FarmerProfile() {
        ...
        const cargarPerfil = async () => {
            try {
                ...
                if (data.certificaciones) {
                    setCertificaciones(data.certificaciones.split(',').map(c => c.trim()));
                }
            } catch (error) {
    ...
    const iniciales = `${profileData.nombres?.charAt(0) || ''}${profileData.apellidoPaterno?.charAt(0) || ''}`.toUpperCase();

    return (
        <div style={{ width: '100%' }}>
            <h2 style={{ color: 'var(--color-primary)', fontFamily: 'var(--font-titles)', marginBottom: '10px', fontSize: '2rem' }}>Mi Perfil</h2>
            <p style={{ color: '#555', fontSize: '1.1rem', marginBottom: '30px' }}>Administra los detalles de tu cuenta y la información visible para los compradores.</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                    ...
                    <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: 'var(--radius-lg)', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '30px' }}>
                            ...
                            <div>
                                <h3 style={{ margin: '0 0 8px 0', color: 'var(--color-text)', fontSize: '1.4rem' }}>{profileData.nombres} {profileData.apellidoPaterno}</h3>
                        ...
                        <form onSubmit={handleDatosPersonalesSubmit}>
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <input type="text" name="nombres" required value={profileData.nombres || ''} onChange={handleProfileChange} style={{ width: '100%', padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid #ccc', fontSize: '1rem' }} />
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                                <div>
                                    ...
                                    <input type="email" disabled value={profileData.email || ''} style={{ width: '100%', padding: '11px', borderRadius: 'var(--radius-md)', border: '1px solid #e0e0e0', backgroundColor: '#f9f9f9', color: '#888', fontSize: '1rem', cursor: 'not-allowed' }} />
                                </div>
                            </div>
                            <button type="submit" style={{ backgroundColor: 'var(--color-primary)', color: 'white', border: 'none', padding: '14px', borderRadius: 'var(--radius-md)', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.05rem', width: '100%', boxShadow: '0 4px 6px rgba(46, 125, 50, 0.2)' }}>
                    ...
                    <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: 'var(--radius-lg)', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                        ...
                        <form onSubmit={handlePasswordSubmit}>
                            ...
                            <button type="submit" style={{ backgroundColor: 'var(--color-primary)', color: 'white', border: 'none', padding: '14px', borderRadius: 'var(--radius-md)', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem', width: '100%', boxShadow: '0 4px 6px rgba(46, 125, 50, 0.2)' }}>
                ...
                <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: 'var(--radius-lg)', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                    ...
                    <form onSubmit={handlePerfilAgricolaSubmit}>
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <textarea name="descripcion" maxLength="500" rows="5" value={profileData.descripcion || ''} onChange={handleProfileChange} placeholder="Escribe algo sobre tu experiencia..." style={{ width: '100%', padding: '14px', borderRadius: 'var(--radius-md)', border: '1px solid #ccc', fontSize: '1rem', fontFamily: 'inherit', resize: 'vertical' }} />
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                            <div>
                                ...
                                <input type="text" name="dniRuc" value={profileData.dniRuc || ''} onChange={handleProfileChange} placeholder="Documento de identidad" style={{ width: '100%', padding: '14px', borderRadius: 'var(--radius-md)', border: '1px solid #ccc', fontSize: '1rem' }} />
                            </div>
                            <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                            <div>
                                ...
                                <input type="number" name="hectareasTotales" value={profileData.hectareasTotales || ''} onChange={handleProfileChange} placeholder="Ej. 50" min="0" step="0.1" style={{ width: '100%', padding: '14px', borderRadius: 'var(--radius-md)', border: '1px solid #ccc', fontSize: '1rem' }} />
                            </div>
                            <div>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', color: '#333' }}>Años de Experiencia</label>
                                <input type="number" name="anosExperiencia" value={profileData.anosExperiencia || ''} onChange={handleProfileChange} placeholder="Ej. 10" min="0" style={{ width: '100%', padding: '14px', borderRadius: 'var(--radius-md)', border: '1px solid #ccc', fontSize: '1rem' }} />
                            </div>
                        </div>

                        {/* CERTIFICACIONES */}
                        <div style={{ marginBottom: '30px', padding: '20px', backgroundColor: '#F4F7F5', borderRadius: 'var(--radius-md)', border: '1px solid #e0e0e0' }}>
                            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', color: '#333', fontSize: '1.05rem' }}>Certificaciones Oficiales</label>
                            <p style={{ fontSize: '0.85rem', color: '#666', marginBottom: '20px' }}>Habilita las certificaciones vigentes con las que cuentas.</p>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                                {CERTIFICACIONES_DISPONIBLES.map((cert, index) => {
                                    const isEnabled = certificaciones.includes(cert);
                                    return (
                                        <div key={index} style={{ padding: '15px', backgroundColor: 'white', borderRadius: 'var(--radius-md)', border: isEnabled ? '1.5px solid var(--color-primary)' : '1px solid #ddd', transition: 'all 0.2s ease' }}>
                                            <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontWeight: '500', color: isEnabled ? 'var(--color-primary)' : '#555', fontSize: '0.95rem' }}>
                                                <input type="checkbox" checked={isEnabled} onChange={() => handleCertToggle(cert)} style={{ width: '18px', height: '18px', cursor: 'pointer', accentColor: 'var(--color-primary)' }} />
                                                {cert}
                                            </label>
                                        </div>
                                    );
                        ...
                        <button type="submit" style={{ backgroundColor: 'var(--color-primary)', color: 'white', border: 'none', padding: '15px', borderRadius: 'var(--radius-md)', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.1rem', width: '100%', boxShadow: '0 4px 6px rgba(46, 125, 50, 0.2)' }}>
